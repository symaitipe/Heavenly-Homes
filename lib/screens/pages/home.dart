...
class _HomePageState extends State<HomePage> {
    ...
    try {
        ...
        for (int index = 0; index < projectsData.length; index++) {
          try {
            ...
            Project project = Project(
              title: projectData['title'] as String? ?? 'Untitled Project',
              imageUrl: imageUrl,
              category: projectData['category'] as String? ?? 'Uncategorized',
              description: projectData['description'] as String? ?? 'No description',
              client: projectData['client'] as String? ?? 'Unknown Client',
              year: (projectData['year'] as num?)?.toInt() ?? 0,
              location: projectData['location'] as String? ?? 'No location',
      ...
      if (mounted) {
        setState(() {
          ...
          _selectedItems = allItems.take(3).toList(); // Limit to 3 items
  ...
  void _startBannerTimer() {
    ...
    _bannerTimer = Timer.periodic(const Duration(seconds: 5), (timer) {
      ...
      int currentPage = _bannerController.page!.round();
      int nextPage = currentPage + 1;
      const int bannerCount = 2;
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  "Failed to load user data. Please restart or log in.",
    ...
    return Container(
      ...
      child: Stack(
        children: [
          ...
          Positioned(
            top: 15,
            right: 60, // Cart Icon Position
            child: StreamBuilder<QuerySnapshot>(
              stream: _currentUserModel?.uid == null
                  ? null
                  : FirebaseFirestore.instance.collection('cart').where('userId', isEqualTo: _currentUserModel!.uid).snapshots(),
          ...
          Positioned(
            top: 15,
            right: 100, // Account Icon Position (Adjust as needed)
            child: IconButton(
              icon: Icon(Icons.account_circle_outlined, color: iconColor, size: 28),
              tooltip: 'My Account',
                  ...
                  leading: ClipRRect(
                    ...
                    child: (item.imageUrl.startsWith('http')
                        ? Image.network(
                            ...
                            height: 40,
                            fit: BoxFit.cover,
                            errorBuilder: (c, e, s) => const Icon(Icons.hide_image_outlined, size: 20),
                          )
                        : Image.asset(
                            item.imageUrl.isNotEmpty ? item.imageUrl : 'assets/placeholder.jpg',
                            width: 40,
                            height: 40,
                            fit: BoxFit.cover,
                            errorBuilder: (c, e, s) => const Icon(Icons.hide_image_outlined, size: 20),
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  ...
                  stops: const [0.2, 0.2], // 20% black, 80% white
            ...
            Text(
              title,
              textAlign: TextAlign.center,
              style: GoogleFonts.poppins(fontSize: 12, color: Colors.black87, height: 1.1),
              maxLines: 2,
            ),
          ],
        ),
  ...
  SliverToBoxAdapter _buildSectionHeader(String title, String viewAllRoute) {
    return SliverToBoxAdapter(
      child: Padding(
        padding: const EdgeInsets.only(left: 19, right: 10, top: 10, bottom: 5),
        child: Row(
      ...
      child: SizedBox(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final String projectId = projectData['projectId'] as String? ?? '';
              ...
              child: GestureDetector(
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => ProjectDetailPage(project: project, projectId: projectId),
                    ),
                  );
                },
                child: SizedBox(
                  ...
                  child: Card(
                    ...
                    child: Column(
                      ...
                      children: [
                        Expanded(
                          child: Image.asset(
                            project.imageUrl.isNotEmpty ? project.imageUrl : 'assets/placeholder.jpg',
                            fit: BoxFit.cover,
                            width: double.infinity,
                        ...
                        Padding(
                          ...
                          child: Text(
                            'LKR ${project.price > 0 ? project.price.toStringAsFixed(0) : 'N/A'}',
  ...
  Widget _buildFeaturedItemsGrid() {
    if (_isLoadingContent && _selectedItems.isEmpty) return const SliverToBoxAdapter(child: SizedBox.shrink());
